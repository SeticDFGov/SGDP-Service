...
public class CtrPermissionServiceTest : CtrTestBase
{
    ...
    {
        Assert.Null(await Permissoes.GetContextAsync("ninguem@df.gov.br"));
    }

    [Fact]
    public async Task GetContext_TrazPapelEPerfil()
    {
        var ctx = await Permissoes.GetContextAsync(UserAnalista.Email);
    ...
    {
        var ctx = (await Permissoes.GetContextAsync(UserAdmin.Email))!;
    ...
    {
        var ctx = (await Permissoes.GetContextAsync(email))!;
    ...
    public async Task PapelPgia_NaoDaAcessoAoModuloDeContratacoes()
    {
        ...
        var ctx = (await Permissoes.GetContextAsync(UserBasico.Email))!;
