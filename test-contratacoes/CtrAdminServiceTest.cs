...
public class CtrAdminServiceTest : CtrTestBase
{
    ...
    [Fact]
    public async Task AtribuirPapel_GravaOPapel()
    {
        ...
        Assert.True(Permissoes.PodeAcessar((await Permissoes.GetContextAsync(user.Email, PerfilDe(user.Email)))!));
    }
    ...
    public async Task AtribuirPapel_NuloRemoveOPapel()
    {
        ...
        Assert.False(Permissoes.PodeAcessar((await Permissoes.GetContextAsync(user.Email, PerfilDe(user.Email)))!));
    }
