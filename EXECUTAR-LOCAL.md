...
2. **Login como a persona "Clara das Contratações"** (ou qualquer usuário com o papel) → cai em `/analises`:
   ...
   - **Lista de processos**: busque por número/órgão/objeto, filtre por categoria, situação, sigla, período e (desde a rodada da chefia) por **etapa do planejamento** e **criticidade**; use "Registrar chegada" para preencher um checkpoint com a data de hoje; exclua um processo (soft delete: some da lista) e confira que o número volta a poder ser cadastrado.
   - **Duas listas e STATUS** (desde 2026-09-24): em Processos, as sub-abas **Processos de supervisão contínua** (os que vieram do órgão comunicante) e **Comunicações do TCDF** (os que nasceram de uma comunicação do TCDF, com o botão "Registrar comunicação do TCDF") separam os processos pela origem, cada uma com os mesmos filtros (o filtro de origem e o de restituição saíram da tela). Troque de uma para a outra e volte: o título da aba do navegador e a sub-aba marcada acompanham. A coluna **Status** mostra "Em regime de supervisão" ou, com o contrato assinado, "Concluída" (marque "Incluir os concluídos" para ver um). A página de um processo do TCDF volta para a lista das comunicações.
   ...
   - **Regra de 2026-09-24**: o critério II pergunta se a contratação impacta diretamente algum projeto de Transformação Digital do órgão (Sim 0, Não 1, Desconhecido 1; sem resposta vale Desconhecido) e o III ficou invertido (Sim 0, Não 1; o padrão continua Não). Abaixo dos critérios aparece a **composição** (pontos de I a VII, a soma e a regra do VII); a página do processo mostra a mesma tabela com os pontos do servidor, e a lista traz os critérios que pontuaram embaixo da soma. Num processo gravado antes (II em Nenhum, Baixo, Médio ou Alto), o II aparece sem opção marcada e com a caixa "Resposta à pergunta anterior"; salvar sem responder mantém a resposta antiga (vale Desconhecido) e responder à pergunta nova a substitui. A migration recalcula a criticidade de todos pela regra nova.
   ...
   - **Esclarecimentos ao órgão**: no formulário do processo, informe a data do pedido e o que foi pedido: a lista e a página passam a mostrar o badge âmbar "Aguardando esclarecimento há N dia(s)", o filtro "aguardando esclarecimento" separa os pendentes e o painel traz o card com o total. Informar a data da resposta encerra a pendência. A **situação do trâmite não muda** com isso (é sinal paralelo), mas os dias sem movimento zeram. Desde 2026-09-24 há também o **Nº do documento SEI do pedido** (opcional; fica desabilitado sem a data do pedido e some junto com ela).
   - **Nº SEI do Formulário e análise técnica** (desde 2026-09-24): na Identificação, o Nº SEI do Formulário usa a máscara do número do processo (opcional; incompleto é recusado). Na Tramitação, o quadro "Encaminhamento para análise técnica" pede a data, a área (SUBSIS ou SUBINFRA, obrigatória com a data), a data do retorno e o que a área respondeu: retorno antes do encaminhamento vai para o quadro vermelho, e limpar a data do encaminhamento limpa o resto. A página do processo e a lista mostram onde a análise técnica está; encaminhar e receber o retorno contam como movimentação ("Parado há").
   - **Restituição** (desde 2026-09-24): o formulário não tem mais a seção "Restituição ao órgão"; processo restituído antes continua Restituído, a página mostra "Registro antigo: restituído ao órgão em ..." e salvar outra coisa não apaga a restituição.
   ...
   - **Exportar CSV**: o arquivo (`supervisao-continua-contratacoes.csv`; na lista das comunicações do TCDF, `supervisao-continua-comunicacoes-tcdf.csv`) abre no Excel sem mojibake e reimporta como "Atualizar" em todas as linhas (os campos de 2026-09-24 não têm coluna e ficam como estão). A coluna do critério II passou a se chamar `Critério II - Impacta projeto de Transformação Digital`; um arquivo exportado antes da regra nova traz a Criticidade antiga e é recusado linha a linha até a coluna Criticidade ser esvaziada ou corrigida. O cabeçalho tem 32 colunas: depois das 4 da rodada da chefia (`Etapa do planejamento;Assinatura do contrato;Criticidade;Origem`) vêm as 3 do esclarecimento, as 7 dos critérios de criticidade (`Critério I - ...` a `Critério VII - ...`) e, no fim, as 3 dos dados da contratação (`Valor estimado (R$);Hospedagem no CeTIC-DF;Usa a rede GDFNet`, com o valor no formato `1234567,89`); o `-` na coluna de retorno ao órgão significa "não se aplica", como já valia na da UGTIC. Concluídos só entram no arquivo com "Incluir os concluídos" marcado.
   - **Painel**: as contagens batem com as duas listas somadas, filtradas pelo mesmo critério (as 8 situações, com Concluído); o clique num número abre a lista dos processos de supervisão contínua, e as comunicações do TCDF ficam na sub-aba delas; o bloco **Concluídos (contrato assinado)** e a relação **Processos concluídos** listam os assinados; os tempos médios só contam processos com as duas datas (inclusive "chegada → assinatura"); a tabela de gargalos ignora concluídos, análise concluída e restituídos.
